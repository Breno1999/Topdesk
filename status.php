<?php
    ...
    if($result['status_atendimento'] == 'Não iniciado'){
        ...
        echo "<script>location.href= 'pag_atendimentos.php';</script>";
            ...
            if($status == 'Concluído'){
                ...
                #tras os valores de acordo com o ID do atendimento
                $cmd = $conn->query("SELECT status_atendimento FROM atendimentos WHERE id_atendimento = '$id'");
                $result = $cmd->fetch();

                if($result['status_atendimento'] == 'Concluído' ){
                    
                    echo "<script>alert('Não é possivel concluir um atendimento duas vezes!');</script>";
                    echo "<script>location.href= 'pag_atendimentos.php';</script>";
